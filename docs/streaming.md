...
| `RTMP_HOMEBREW_DIRECT_PLAYER` | `direct_player` | Enables Bonfire's self-hosted Vidstack player. Production deployments default to `true`; set it to `false` for the iframe rollback path. |
...
Production playback defaults to Bonfire's direct player after deployment.
The direct player uses self-hosted Vidstack 0.6.15 (MIT) and hls.js 1.7.0
(Apache-2.0), sends grants only as bearer headers, and retains RTMP Homebrew's
embeddable version-1 `postMessage` player as the rollback mode. Deploy with
`RTMP_HOMEBREW_DIRECT_PLAYER=false` to activate that fallback. Development
Basic credentials remain in RTMP Homebrew and are never copied into Bonfire.
