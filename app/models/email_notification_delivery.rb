class EmailNotificationDelivery < ApplicationRecord
  ...
  enum :kind, %w[ mention daily_summary new_user_signup stream_live ].index_by(&:itself), prefix: true
  ...
  validates :room, :stream_session_id, presence: true, if: :kind_stream_live?
end
